...
Every minted MiningAgent NFT is simultaneously proof of AI:
...
- **AI verification gate:** Minting requires LLM to solve SMHL within 20s (proves AI capability). ERC-721 NFT ownership is the mining gate. PoW hash includes `msg.sender`, preventing nonce transfer. `tx.origin` check blocks contract callers.
...
contracts/
  src/
    ...
    MiningAgent.sol        ERC-721 mining rig NFTs (proof of AI)
...
- [ ] Proof of AI: `setAgentURI()`, `setMetadata()`, `getAgentWallet()` all functional
...
- [ERC-721](https://eips.ethereum.org/EIPS/eip-721): Non-Fungible Token (mining rigs with on-chain metadata and wallet binding)
